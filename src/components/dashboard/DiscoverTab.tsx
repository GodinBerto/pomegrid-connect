...
export const DiscoverTab = () => {
  ...
  const partnersQuery = useQuery({
    queryKey: ["discover-partners"],
    queryFn: fetchPartners,
  });

  const partners = partnersQuery.data ?? [];
  const relevantPartners = partners.filter((partner) => {
    if (user?.role === "farmer") return partner.type === "importer";
    if (user?.role === "importer") return partner.type === "farmer";
    return true;
  });

  const filtered = relevantPartners.filter((partner) => {
